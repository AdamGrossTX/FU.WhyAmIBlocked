...
function Get-FUClientSafeguardHoldInfo {
    param(
...
function Get-FUSafeGuardHoldDetails {
    Param (
        [parameter(Mandatory = $true)]
        [string]$AppraiserURL,
        ...
        [parameter(Mandatory = $false)]
        [string]$Path = "C:\FeatureUpdateBlocks"
    )

    $AppriaserRoot = $Path

    $ExistingXML = Get-ChildItem -Path $AppriaserRoot\*.xml -Recurse -File | Where-Object { $_.Name -like "*$AppraiserVersion*" -and $_.Name -notlike "*_ORIG.xml" } -ErrorAction SilentlyContinue
...
$Settings = Get-FUClientSafeguardHoldInfo -OS "NI22H2"

#Run this with a list of ids to get specific entries
Get-FUSafeGuardHoldDetails -AppraiserURL $Settings.ALTERNATEDATALINK -AppraiserVersion $Settings.ALTERNATEDATAVERSION -SafeGuardHoldId $Settings.GatedBlockId 

#Run this with no ids to list all safeguard holds from the appraiser db
$Blocks = Get-FUSafeGuardHoldDetails -AppraiserURL $Settings.ALTERNATEDATALINK -AppraiserVersion $Settings.ALTERNATEDATAVERSION
